...
let currentLang = localStorage.getItem('elite-casino-lang') || 'en';
...
function setActiveLang(lang) {
  ...
  promosTitle.textContent = lang === 'ar' ? 'عروض النخبة الحصرية' : 'Exclusive Elite Promotions';
}
...
function setAffiliateAccordionText(lang, open) {
  ...
  if (!btn) return;
  if (lang === 'ar') {
    btn.textContent = open ? 'إخفاء شراكة بيترا.بت النخبة' : 'عرض شراكة بيترا.بت النخبة';
  } else {
    btn.textContent = open ? 'Hide Petra.bet Elite Partnership' : 'Show Petra.bet Elite Partnership';
  }
}
